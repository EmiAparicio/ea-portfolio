...
const Resume: FC<ResumeProps> = ({ className, loaderDelayMs = 180 }) => {
  ...
  const src = useMemo(
    () =>
      isEsp
        ? '/cv/EmilianoAparicio-CV-Eng-ES.pdf'
        : '/cv/EmilianoAparicio-CV-Eng-EN.pdf',
    [isEsp]
  );
  ...
  return (
    <div className="pointer-events-none relative h-full w-full">
      ...
      <HexButton
        id="cv-download"
        title={dict.pages.eng.downloadCv}
        label={dict.pages.eng.downloadCvLabel}
        icon={FaFileDownload}
        sizeFactor={1.5}
        iconScale="40%"
        labelSizeFactor={0.6}
        ...
        href={
          lang === 'en'
            ? '/cv/EmilianoAparicio-CV-Eng.pdf'
            : '/cv/EmilianoAparicio-CV-Eng.pdf'
        }
        download={true}
        toggleable={false}
